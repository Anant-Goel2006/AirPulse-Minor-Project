...
function buildLiveImageUrl(seed, width = 1600, height = 900, fallbackTerms = 'city skyline') {
  const parts = [seed, fallbackTerms, 'air quality']
    .flatMap(item => String(item || '').split(/[,\s]+/))
    .map(item => item.trim())
    .filter(Boolean);
  const uniqueParts = [...new Set(parts)];
  const query = encodeURIComponent(uniqueParts.join(' ') || fallbackTerms);
  const sig = uniqueParts.join(' ').split('').reduce((acc, ch, idx) => (acc + ch.charCodeAt(0) * (idx + 1)) % 9973, 0) || 1;
  return `https://source.unsplash.com/featured/${width}x${height}/?${query}&sig=${sig}`;
}
...
let currentResolvedStation = null;
...
let pollutantGraphLoadSeq = 0;
let forecastCoachLoadSeq = 0;
...
let gaugeValueAnimatedValue = null;
...
function parseGeoQuery(raw) {
  const match = String(raw || '').trim().match(/^geo:\s*(-?\d+(?:\.\d+)?)\s*;\s*(-?\d+(?:\.\d+)?)$/i);
  ...
  return {
    lat,
    lng,
    query: `geo:${lat.toFixed(6)};${lng.toFixed(6)}`,
  };
}
...
function buildHeroLocationHint(meta = {}, displayHint = '', stationHint = '') {
  const area = normalizeDisplayName(meta?.area || '');
  const city = normalizeDisplayName(meta?.city || '');
  const country = normalizeDisplayName(meta?.country || '');
  const display = normalizeDisplayName(displayHint || '');
  const station = normalizeDisplayName(stationHint || '');
  const candidates = [
    [area, city, country].filter(Boolean).join(' '),
    [display, city, country].filter(Boolean).join(' '),
    [station, city, country].filter(Boolean).join(' '),
...
function locationMetaFromLiveData(data, fallbackQuery = '', displayHint = '') {
  const userLocation = normalizeUserLocationMeta(
    data?.user_location,
    displayHint || fallbackQuery || curCityDisplay || curCity,
    {
      lat: Number(data?.city?.geo?.[0]),
      lng: Number(data?.city?.geo?.[1]),
    }
  );
  const stationName = String(
    userLocation?.label
    || data?.city?.name
    || data?.resolved_station_name
    || data?.station_name
    || displayHint
    || fallbackQuery
    || ''
  ).trim();
  const parsed = parseMapStationLocation(
    stationName,
    userLocation?.city || displayHint || fallbackQuery || curCityDisplay || curCity
  );
  return enrichLocationMeta({
    city: normalizeDisplayName(userLocation?.city || parsed.city || displayHint || fallbackQuery || curCityDisplay || curCity),
    state: normalizeDisplayName(userLocation?.state || data?.state || ''),
    country: normalizeDisplayName(userLocation?.country || data?.country || parsed.country || ''),
    area: normalizeDisplayName(userLocation?.area || parsed.area || data?.area || ''),
  }, userLocation?.city || fallbackQuery || displayHint || curCityDisplay || curCity);
}
...
function resolvedStationMatchesQuery(query) {
  const key = snapshotKey(query);
  if (!key || !currentResolvedStation) return false;
  return [
    currentResolvedStation.query,
    currentResolvedStation.stationQuery,
    currentResolvedStation.geoQuery,
  ].some(value => snapshotKey(value) === key);
}
...
function normalizeResolvedStationMeta(meta = null, fallbackLabel = '') {
  if (!meta || typeof meta !== 'object') return null;
  const rawQuery = String(meta.query || '').trim();
  const uidRaw = String(meta.uid ?? '').trim().replace(/^@+/, '');
  const uid = Number(uidRaw);
  const lat = Number(meta.lat);
  const lng = Number(meta.lng);
  const stationName = String(meta.station_name || meta.stationName || fallbackLabel || '').trim();
  const distanceKm = Number(meta.distance_km ?? meta.distanceKm);
  const stationQuery = rawQuery || (Number.isFinite(uid) ? `@${uid}` : '');
  return {
    uid: Number.isFinite(uid) ? uid : null,
    query: stationQuery,
    stationQuery,
    geoQuery: String(meta.geoQuery || meta.geo_query || '').trim(),
    lat: Number.isFinite(lat) ? lat : null,
    lng: Number.isFinite(lng) ? lng : null,
    station_name: stationName,
    distance_km: Number.isFinite(distanceKm) ? distanceKm : null,
    estimated: Boolean(meta.estimated),
    match_mode: String(meta.match_mode || meta.matchMode || '').trim(),
    stations_considered: Number.isFinite(Number(meta.stations_considered ?? meta.stationsConsidered))
      ? Number(meta.stations_considered ?? meta.stationsConsidered)
      : null,
  };
}

function normalizeUserLocationMeta(meta = null, fallbackLabel = '', coords = {}) {
  const source = (meta && typeof meta === 'object') ? meta : {};
  const label = normalizeDisplayName(source.label || source.displayName || fallbackLabel || '');
  const area = normalizeDisplayName(source.area || '');
  const city = normalizeDisplayName(source.city || '');
  const state = normalizeDisplayName(source.state || '');
  const country = normalizeDisplayName(source.country || '');
  const lat = Number(source.lat ?? source.latitude ?? coords.lat);
  const lng = Number(source.lng ?? source.lon ?? source.longitude ?? coords.lng);
  const estimated = Boolean(source.estimated);
  const stationsConsidered = Number(source.stations_considered ?? source.stationsConsidered);
  const radiusKm = Number(source.radius_km ?? source.radiusKm);
  if (!label && !area && !city && !Number.isFinite(lat) && !Number.isFinite(lng)) return null;
  return {
    label: label || area || city || normalizeDisplayName(fallbackLabel) || 'Your location',
    area,
    city,
    state,
    country,
    ...
    lng: Number.isFinite(lng) ? lng : null,
    estimated,
    match_mode: String(source.match_mode || source.matchMode || '').trim(),
    stations_considered: Number.isFinite(stationsConsidered) ? stationsConsidered : null,
    radius_km: Number.isFinite(radiusKm) ? radiusKm : null,
  };
}

function applyResolvedStationToLiveData(payload, stationMeta = null, displayLabel = '', options = {}) {
  const normalized = normalizeResolvedStationMeta(stationMeta, displayLabel);
  const incomingLocation = options?.userLocation || payload?.user_location || payload?.data?.user_location || null;
  const userLocation = normalizeUserLocationMeta(incomingLocation, displayLabel, {
    lat: options?.lat,
    lng: options?.lng,
  });
  if (!payload || typeof payload !== 'object') return payload;
  const copy = clonePayloadSafe(payload) || payload;
  const data = (copy.data && typeof copy.data === 'object') ? copy.data : copy;
  if (!data || typeof data !== 'object') return copy;
  if (!data.city || typeof data.city !== 'object') data.city = {};
  if (userLocation) {
    data.user_location = {
      ...
      estimated: userLocation.estimated,
      match_mode: userLocation.match_mode,
      stations_considered: userLocation.stations_considered,
      radius_km: userLocation.radius_km,
    };
  }
  if (Number.isFinite(userLocation?.lat) && Number.isFinite(userLocation?.lng)) {
    data.city.geo = [userLocation.lat, userLocation.lng];
  } else if (Number.isFinite(normalized.lat) && Number.isFinite(normalized.lng)) {
    data.city.geo = [normalized.lat, normalized.lng];
  }
  if (!String(data.city.name || '').trim() && userLocation?.label) {
    data.city.name = userLocation.label;
  }
  if (normalized?.station_name) {
    data.resolved_station_name = normalized.station_name;
  }
  if (normalized) {
    data.resolved_station = {
      uid: normalized.uid,
      query: normalized.stationQuery || normalized.query,
      lat: normalized.lat,
      lng: normalized.lng,
...
function extractLivePollutants(data) {
  const iaqi = data?.iaqi || data?.pollutants || {};
  const out = {};
...
function buildNearbyAreaRequestUrl(query, limit = 140, radiusKm = 32) {
  const hasCoords = Number.isFinite(Number(currentLiveLocation?.lat)) && Number.isFinite(Number(currentLiveLocation?.lng));
  if (hasCoords) {
    const params = new URLSearchParams({
      lat: String(Number(currentLiveLocation.lat)),
      lng: String(Number(currentLiveLocation.lng)),
      limit: String(limit),
      radius_km: String(radiusKm),
      label: String(currentLiveLocation?.label || query || curCityDisplay || curCity || '').trim(),
    });
    return `/api/live/areas?${params.toString()}`;
  }
  return `/api/live/areas/${encodeURIComponent(query)}?limit=${encodeURIComponent(limit)}&radius_km=${encodeURIComponent(radiusKm)}`;
}
...
function bindAreaSliderControls() { /* Ticker is automated now */ }
...
function renderAreaAqiState(msg) {
  ...
  if (metaEl && !String(msg || '').toLowerCase().includes('loading')) {
    metaEl.textContent = '';
  }
...
function renderAreaAqiList(rows, centerName = '') {
  ...
  const items = rows
    .map(item => ({
      ...
      station: String(item?.station_name || item?.station || '').trim(),
  ...
  if (metaEl) {
    ...
    metaEl.textContent = `${items.length} live areas${label}`;
  }

  const shouldAnimate = listEl.classList.contains('ticker-content') && items.length > 5;
  const renderItems = shouldAnimate ? [...items, ...items] : items;
  listEl.style.animation = shouldAnimate ? '' : 'none';
  listEl.innerHTML = renderItems.map(item => {
    ...
    if (Number.isFinite(item.distance)) secondaryParts.push(`${item.distance.toFixed(item.distance < 10 ? 1 : 0)} km`);
    ...
    return `<button class="area-aqi-chip" data-uid="${escapeHtml(item.uid)}" data-station="${escapeHtml(item.station)}" data-display="${escapeHtml(primary)}" data-bghint="${escapeHtml(item.station)}" title="${escapeHtml(item.station)}">
      <span class="area-aqi-badge" style="background:${cat.color};color:${cat.textClr}">AQI ${Math.round(item.aqi)}</span>
...
async function loadAreaAqiList(cityQuery, reqSeq = null) {
  ...
  const useCoords = Number.isFinite(Number(currentLiveLocation?.lat)) && Number.isFinite(Number(currentLiveLocation?.lng));
  const cacheKey = useCoords ? '' : getAreaCacheKey(query);
  if (cacheKey) {
  ...
  renderAreaAqiState('Loading nearby AQI...');
  try {
    let fetchPromise = null;
    if (cacheKey) {
      fetchPromise = areaListInFlight.get(cacheKey);
      if (!fetchPromise) {
        fetchPromise = fetchJsonNoCache(buildNearbyAreaRequestUrl(query))
          .finally(() => {
    ...
    if (cacheKey) storeAreaResponseCache(query, { areas: r.areas, centerName });
...
function buildSeededHeroImage(seed) {
  return buildLiveImageUrl(seed || 'city skyline', 1920, 1080, 'city skyline atmosphere');
}
...
function buildLocalityPhotoUrl(labelText, countryLabel = '') {
  ...
  return buildLiveImageUrl(`${raw} ${countryLabel}`.trim() || 'city skyline', 480, 300, 'city street skyline');
}

async function fetchDynamicHeroImage(seedText) {
  const seed = String(seedText || '').trim();
  if (!seed) return null;
  try {
    const j = await fetchJsonNoCache(`/api/city-image/${encodeURIComponent(seed)}`);
    if (!j?.error && String(j?.url || '').trim()) {
      let imageUrl = String(j.url).trim();
      try {
        const parsed = new URL(imageUrl, window.location.origin);
        const host = String(parsed.hostname || '').toLowerCase();
        if (host.includes('pexels.com') || host.includes('unsplash.com')) {
          parsed.searchParams.set('apv', slugifyCity(seed) || 'hero');
...
async function resolveHeroBg(cityName, countryName, queryHint = '') {
  ...
  const dynamicSeedCandidates = [
    hintText,
    [hintText, countryText].filter(Boolean).join(' '),
    [parsedHint?.area || '', hintedCity, countryText].filter(Boolean).join(' '),
    cityText,
    [cityText, countryText].filter(Boolean).join(' '),
    hintedCity,
    [hintedCity, countryText].filter(Boolean).join(' '),
  ].map(item => String(item || '').trim()).filter(Boolean);

  for (const seedText of [...new Set(dynamicSeedCandidates)]) {
    resolved = await fetchDynamicHeroImage(seedText);
    if (resolved) break;
  }

  // Fall back to bundled city imagery only when a live image cannot be resolved.
  if (!resolved) {
  ...
  if (!resolved) {
    const seed = hintText || cityText || hintedCity || countryText || countryKey;
    if (seed) {
      resolved = await fetchDynamicHeroImage(seed);
      if (!resolved) {
        resolved = { imageUrl: buildLiveImageUrl(seed, 1920, 1080, 'city skyline atmosphere'), focalPoint: 'center' };
      }
    }
  }
...
async function updateCinematicHero({ cityName, country, aqi, level, updatedAt, timeIso, bgHint = '' }, reqSeq = null) {
  if (isStaleReq(reqSeq)) return;
  const hero = $('cinematicHero');
  if (!hero) return;
  ...
  const bgSeed = normalizeDisplayName(bgHint || curHeroQueryHint || displayCity || curCityDisplay || curCity);
  const bgCfg = await resolveHeroBg(displayCity, displayCountry, bgSeed);
  if (isStaleReq(reqSeq)) return;
...
async function fetchLivePayloadForQuery(query, options = {}) {
  const rawQuery = String(query || '').trim();
  if (!rawQuery) return { error: 'Missing live query', status: 0 };
  const geo = parseGeoQuery(rawQuery);
  if (!geo) {
    return fetchJsonNoCache(`/api/live/${encodeURIComponent(rawQuery)}${options.fresh === false ? '' : '?fresh=1'}`);
  }
  const params = new URLSearchParams({
    lat: Number(geo.lat).toFixed(6),
    lng: Number(geo.lng).toFixed(6),
  });
  const label = String(options.label || currentLiveLocation?.label || '').trim();
  if (label) params.set('label', label);
  const accuracy = Number(options.accuracy ?? currentLiveLocation?.accuracy);
  if (Number.isFinite(accuracy) && accuracy > 0) params.set('accuracy_m', String(Math.round(accuracy)));
  return fetchJsonNoCache(`/api/live/nearby?${params.toString()}`);
}

async function loadLiveFromCoords(lat, lng, modeLabel = 'your location', accuracyM = null) {
  ...
  const geoQuery = `geo:${latStr};${lngStr}`;

  try {
    const nearby = await fetchLivePayloadForQuery(geoQuery, {
      label: currentLiveLocation?.label || modeLabel,
      accuracy: accuracyM,
    });
    if (nearby?.status === 'ok' && nearby?.data) {
      const locationMeta = normalizeUserLocationMeta(nearby?.data?.user_location, modeLabel, { lat, lng });
      const stationName = nearby?.data?.resolved_station_name || nearby?.nearest?.station_name || nearby?.data?.city?.name || '';
      const displayName = normalizeDisplayName(locationMeta?.label || modeLabel || 'Your location');
      const resolvedStation = normalizeResolvedStationMeta(nearby?.nearest || nearby?.data?.resolved_station, stationName || displayName || modeLabel);
      const resolvedData = applyResolvedStationToLiveData(nearby.data, resolvedStation, displayName, {
        lat,
        lng,
        userLocation: locationMeta,
      });
      currentResolvedStation = resolvedStation
        ? {
          ...resolvedStation,
          query: geoQuery,
          geoQuery,
          stationQuery: resolvedStation.stationQuery || resolvedStation.query || '',
        }
        : {
          query: geoQuery,
          geoQuery,
      ...
      currentLiveQuery = geoQuery;
      currentLiveLocation = {
        lat,
        lng,
        label: displayName,
        accuracy: Number.isFinite(Number(accuracyM)) ? Number(accuracyM) : null,
        estimated: Boolean(locationMeta?.estimated),
      };
      curCity = geoQuery;
      curCityDisplay = displayName;
      curHeroQueryHint = buildHeroLocationHint(locationMeta || {}, displayName, stationName) || normalizeDisplayName(locationMeta?.city || displayName) || displayName;
      currentUserCity = normalizeDisplayName(locationMeta?.city || parseMapStationLocation(stationName || displayName || geoQuery, displayName || geoQuery).city) || displayName;
      rememberLiveSnapshot(
        [geoQuery, currentResolvedStation?.stationQuery, displayName, stationName, nearby?.data?.city?.name, locationMeta?.city],
        resolvedData
      );
      if (!applyCachedLiveSnapshot(resolvedData, reqSeq, displayName || modeLabel || '')) return false;
      const focusLat = Number(locationMeta?.lat ?? resolvedData?.city?.geo?.[0]);
      const focusLng = Number(locationMeta?.lng ?? resolvedData?.city?.geo?.[1]);
      if (aqiMap && Number.isFinite(focusLat) && Number.isFinite(focusLng)) {
        aqiMap.setView([focusLat, focusLng], Math.max(aqiMap.getZoom(), 11));
      }
      const d = Number(resolvedStation?.distance_km);
      const stationsUsed = Number(locationMeta?.stations_considered);
      if (locationMeta?.estimated) {
        const countLabel = Number.isFinite(stationsUsed) && stationsUsed > 1 ? ` using ${stationsUsed} nearby stations` : '';
        toast(`Showing a live area estimate${countLabel} for ${displayName}`, 'info');
        setLocationAutoStatus('Using live area average', 'info');
        setLocationSummary(`Live AQI estimated around ${displayName} from nearby monitoring stations.`, 'info');
      } else if (Number.isFinite(d)) {
        toast(`Nearest station is ${d.toFixed(1)} km from ${modeLabel}`, 'success');
        setLocationAutoStatus(`Using ${modeLabel}`, 'success');
        setLocationSummary(`Live AQI resolved for ${displayName}.`, 'success');
      } else {
        toast(`Loaded live AQI for ${modeLabel}`, 'success');
        setLocationAutoStatus(`Using ${modeLabel}`, 'success');
        setLocationSummary(`Live AQI resolved for ${displayName}.`, 'success');
      }
      showLocationAqiPopup(lat, lng, resolvedData);
      return true;
    }
  } catch { }
  ...
  try {
    ...
    if (geo?.status === 'ok' && geo?.data) {
      ...
      const fallbackLocation = normalizeUserLocationMeta(null, modeLabel, { lat, lng });
      const displayName = normalizeDisplayName(fallbackLocation?.label || stationName || modeLabel || 'Your location');
      const resolvedData = applyResolvedStationToLiveData(geo.data, null, displayName, {
        lat,
        lng,
        userLocation: fallbackLocation,
      });
      currentResolvedStation = { query: geoQuery, geoQuery, stationQuery: '' };
      currentLiveQuery = geoQuery;
      currentLiveLocation = {
        lat,
        lng,
      ...
      curCity = geoQuery;
      curCityDisplay = displayName;
      curHeroQueryHint = buildHeroLocationHint(fallbackLocation || {}, displayName, stationName) || displayName;
      currentUserCity = normalizeDisplayName(parseMapStationLocation(stationName || displayName || geoQuery, displayName || geoQuery).city) || displayName;
      rememberLiveSnapshot([geoQuery, displayName, stationName, geo?.data?.city?.name], resolvedData);
      if (!applyCachedLiveSnapshot(resolvedData, reqSeq, displayName || modeLabel || '')) return false;
      const focusLat = Number(resolvedData?.city?.geo?.[0]);
      const focusLng = Number(resolvedData?.city?.geo?.[1]);
      if (aqiMap && Number.isFinite(focusLat) && Number.isFinite(focusLng)) {
        aqiMap.setView([focusLat, focusLng], Math.max(aqiMap.getZoom(), 11));
      }
      toast(`Loaded live AQI for ${modeLabel}`, 'success');
  ...
  if (nearest?.city) {
    currentResolvedStation = null;
    if (aqiMap) aqiMap.setView([Number(nearest.lat), Number(nearest.lng)], 10);
    await loadCity({
      query: geoQuery,
      displayName: normalizeDisplayName(nearest.city || modeLabel || 'Your location'),
      bgHint: normalizeDisplayName(nearest.city || modeLabel || 'Your location'),
      lat,
      lng,
    });
...
function formatAccuracyLabel(meters) {
...
function getBrowserPosition(timeoutMs = 14000, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const actualTimeout = Math.max(5000, Math.min(Number(timeoutMs) || 14000, 20000));
    const desiredAccuracy = Number.isFinite(Number(options.desiredAccuracy))
      ? Math.max(100, Number(options.desiredAccuracy))
      : 1500;
    const bestEffortWaitMs = Number.isFinite(Number(options.bestEffortWaitMs))
      ? Math.max(3000, Math.min(Number(options.bestEffortWaitMs), actualTimeout))
      : Math.min(actualTimeout, 9000);
    ...
    let bestEffortTimer = null;

    const cleanup = () => {
      if (timeoutTimer) clearTimeout(timeoutTimer);
      if (bestEffortTimer) clearTimeout(bestEffortTimer);
      if (watchId != null && navigator.geolocation?.clearWatch) {
        try { navigator.geolocation.clearWatch(watchId); } catch { }
      }
    };

    const finish = (result = null, err = null) => {
      if (settled) return;
      settled = true;
      cleanup();
      if (result) {
      ...
      reject(err || new Error('Geolocation timeout'));
    };

    const considerPosition = (pos) => {
      if (!pos) return;
      const accuracy = getPositionAccuracy(pos);
      if (!bestPos || accuracy < getPositionAccuracy(bestPos)) {
        bestPos = pos;
      }
    ...
    bestEffortTimer = setTimeout(() => {
      if (bestPos) finish(bestPos, null);
    }, bestEffortWaitMs);

    try {
      watchId = navigator.geolocation.watchPosition(
        (pos) => considerPosition(pos),
        (err) => {
...
async function locateUser(options = {}) {
  ...
  const desiredAccuracy = Number.isFinite(Number(options.desiredAccuracy))
    ? Number(options.desiredAccuracy)
    : (startup ? 3000 : 1800);
  const maxBrowserAccuracy = Number.isFinite(Number(options.maxBrowserAccuracy))
    ? Number(options.maxBrowserAccuracy)
    : (startup ? 15000 : 12000);
  const hardRejectAccuracy = Number.isFinite(Number(options.hardRejectAccuracy))
    ? Number(options.hardRejectAccuracy)
    : 25000;
  ...
  try {
    const pos = await getBrowserPosition(timeoutMs, {
      desiredAccuracy,
      bestEffortWaitMs: startup ? 7000 : 10000,
    });
    const lat = Number(pos?.coords?.latitude);
    const lng = Number(pos?.coords?.longitude);
    const accuracy = Number(pos?.coords?.accuracy);
    if (Number.isFinite(lat) && Number.isFinite(lng)) {
      if (!startup && Number.isFinite(accuracy) && accuracy > hardRejectAccuracy) {
        throw Object.assign(new Error('Location accuracy too low'), { code: 'coarse_accuracy', accuracy });
      }
      const modeLabel = (!startup && Number.isFinite(accuracy) && accuracy > maxBrowserAccuracy)
        ? 'your approximate location'
        : 'your location';
      const ok = await loadLiveFromCoords(lat, lng, modeLabel, accuracy);
      if (ok && currentLiveLocation && Number.isFinite(accuracy)) {
        currentLiveLocation.accuracy = accuracy;
      }
      if (ok && !startup && allowToast && Number.isFinite(accuracy) && accuracy > maxBrowserAccuracy) {
        toast(`Browser precision is limited (~${formatAccuracyLabel(accuracy)}). Showing the closest live station to the best detected position.`, 'info');
      }
      return ok;
    }
  } catch (err) {
    const msg = err?.code === 1
      ...
      : (err?.code === 'coarse_accuracy'
        ? `Your browser location is still too coarse${Number.isFinite(Number(err?.accuracy)) ? ` (~${formatAccuracyLabel(Number(err.accuracy))})` : ''}. Turn on precise/high-accuracy location and try again.`
...
async function handleLocateClick(triggerBtn = null) {
  ...
  const ok = await locateUser({ allowToast: true, startup: false, allowApproximate: false });
...
function showLocationAqiPopup(lat, lng, data) {
  try {
    ...
    const resolvedStation = normalizeResolvedStationMeta(data?.resolved_station || currentResolvedStation, data?.resolved_station_name || data?.city?.name || '');
    const userLocation = normalizeUserLocationMeta(data?.user_location, 'Your location', { lat, lng });
    const stationLat = Number(resolvedStation?.lat ?? data.city?.geo?.[0]);
    const stationLng = Number(resolvedStation?.lng ?? data.city?.geo?.[1]);
    ...
    const dist = Number.isFinite(resolvedStation?.distance_km)
      ? Number(resolvedStation.distance_km)
      : (Number.isFinite(stationLat) && Number.isFinite(stationLng)
        ? haversineKm(Number(lat), Number(lng), stationLat, stationLng)
        : null);
    const stationsUsed = Number(userLocation?.stations_considered);
    const proximity = userLocation?.estimated
      ? `${Number.isFinite(stationsUsed) && stationsUsed > 1 ? `${stationsUsed} nearby stations averaged` : 'Nearby stations averaged'} around your detected area`
      : (Number.isFinite(dist) ? `${dist.toFixed(1)} km from detected location` : 'Nearest station to detected location');
    const popupTitle = userLocation?.label || resolvedStation?.station_name || data?.resolved_station_name || data.city?.name || 'Nearby station';
    const stationLine = resolvedStation?.station_name && normalizeCityKey(resolvedStation.station_name) !== normalizeCityKey(popupTitle)
      ? `<div style="font-size:.75rem;color:#475569;margin-bottom:6px">Matched station: ${escapeHtml(resolvedStation.station_name)}</div>`
      : '';

    const html = `<div style="font-family:'Plus Jakarta Sans',sans-serif;min-width:200px">
      <div style="font-size:1rem;font-weight:800;color:#1a1d2e">${escapeHtml(popupTitle)}</div>
...
function applyCachedLiveSnapshot(snapshot, reqSeq, displayNameHint = '') {
  ...
  const forecastCity = displayNameHint || curCityDisplay || currentUserCity || curCity;
  loadAreaAqiList(forecastCity, reqSeq).catch(() => { });
...
async function loadCity(cityInput) {
  const { query: city, displayName, bgHint, lat, lng } = normalizeLoadCityInput(cityInput);
  if (!city) return;
  ...
  const keepResolvedStation = resolvedStationMatchesQuery(city);
  if (!keepResolvedStation) currentResolvedStation = null;
  const geoPoint = parseGeoQuery(city);
  const preserveLiveLocation = Number.isFinite(lat) || Number.isFinite(lng) || Boolean(geoPoint) || keepResolvedStation;
  const nextLat = Number.isFinite(lat)
    ? lat
    : (Number.isFinite(Number(geoPoint?.lat))
      ? Number(geoPoint.lat)
      : (preserveLiveLocation ? Number(currentLiveLocation?.lat) : Number.NaN));
  const nextLng = Number.isFinite(lng)
    ? lng
    : (Number.isFinite(Number(geoPoint?.lng))
      ? Number(geoPoint.lng)
      : (preserveLiveLocation ? Number(currentLiveLocation?.lng) : Number.NaN));
  const nextLabel = normalizeDisplayName(displayName || previewName || currentLiveLocation?.label || city);
  curCity = city;
  currentLiveQuery = city;
  currentLiveLocation = Number.isFinite(nextLat) && Number.isFinite(nextLng)
    ? {
      ...(currentLiveLocation || {}),
      lat: nextLat,
      lng: nextLng,
      label: nextLabel || currentLiveLocation?.label || 'Your location',
    }
    : null;
  ...
  try {
    const j = await fetchLivePayloadForQuery(city, {
      label: displayName || previewName || currentLiveLocation?.label || '',
      accuracy: currentLiveLocation?.accuracy,
      fresh: true,
    });
    ...
    curLiveData = resolvedStationMatchesQuery(city)
      ? applyResolvedStationToLiveData(j.data, currentResolvedStation, displayName || previewName || city, {
        lat: currentLiveLocation?.lat,
        lng: currentLiveLocation?.lng,
      })
      : j.data;
    updateLungs();
    const resolvedAqi = resolveLiveAqi(curLiveData, null);
    if (!Number.isFinite(Number(curLiveData?.aqi)) && Number.isFinite(resolvedAqi)) {
      curLiveData.aqi = resolvedAqi;
    }
    curTimeIso = curLiveData?.time?.iso || '';
    if ($('aqiUpdated')) $('aqiUpdated').textContent = 'Updated: ' + new Date().toLocaleTimeString();
    if (!displayName && !isDirectCityQuery(city)) {
      const liveLabel = normalizeDisplayName(curLiveData?.resolved_station_name || curLiveData?.city?.name || '');
      if (liveLabel) curCityDisplay = liveLabel;
    }
    if (!bgHint && !isDirectCityQuery(city)) {
      const liveHint = normalizeDisplayName(curLiveData?.resolved_station_name || curLiveData?.city?.name || curCityDisplay || previewName);
      if (liveHint) curHeroQueryHint = liveHint;
    }
    rememberLiveSnapshot(
      [city, displayName, bgHint, previewName, curCityDisplay, curLiveData?.resolved_station_name, curLiveData?.city?.name],
      curLiveData
    );
    renderHero(curLiveData, reqSeq, displayName || curCityDisplay || previewName);
    syncLocationSelectionFromData(curLiveData, city, displayName || curCityDisplay || previewName);
    const liveLat = Number(curLiveData?.city?.geo?.[0]);
    const liveLng = Number(curLiveData?.city?.geo?.[1]);
    if (aqiMap && Number.isFinite(liveLat) && Number.isFinite(liveLng)) {
      aqiMap.setView([liveLat, liveLng], Math.max(aqiMap.getZoom(), 10));
    ...
    const analyticsCity = normalizeDisplayName(
      parseMapStationLocation(curLiveData?.resolved_station_name || curLiveData?.city?.name || curCityDisplay || city, curCityDisplay || city).city
    ) || curCityDisplay || city;
...
async function loadLocalAqi(cityOverride = null, reqSeq = null) {
  try {
    ...
    if (resolvedStationMatchesQuery(cityToLoad)) {
      curLiveData = applyResolvedStationToLiveData(curLiveData, currentResolvedStation, d.station_name || d.city || cityToLoad, {
        lat: currentLiveLocation?.lat,
        lng: currentLiveLocation?.lng,
      });
    }
    updateLungs();
    const fbLat = Number(resolvedStationMatchesQuery(cityToLoad)
      ? (currentLiveLocation?.lat ?? currentResolvedStation?.lat)
      : d.latitude);
    const fbLng = Number(resolvedStationMatchesQuery(cityToLoad)
      ? (currentLiveLocation?.lng ?? currentResolvedStation?.lng)
      : d.longitude);
    if (aqiMap && Number.isFinite(fbLat) && Number.isFinite(fbLng)) {
      aqiMap.setView([fbLat, fbLng], Math.max(aqiMap.getZoom(), 9));
    }
    if ($('aqiUpdated')) $('aqiUpdated').textContent = 'Updated: ' + new Date().toLocaleTimeString();
    const fallbackLabel = normalizeDisplayName(
      (resolvedStationMatchesQuery(cityToLoad) ? currentLiveLocation?.label : '')
      || d.city
      || cityToLoad
    );
    if (fallbackLabel) {
    ...
    loadDonut();
    loadNlpAdvice(curLiveData, reqSeq);
    loadAreaAqiList(`${currentLiveLocation?.label || d.city || cityToLoad}`, reqSeq);
    const analyticsCity = normalizeDisplayName(
      (resolvedStationMatchesQuery(cityToLoad) ? currentLiveLocation?.label : '')
      || d.city
      || cityToLoad
...
function renderHero(data, reqSeq = null, displayNameHint = '') {
  ...
  const heroHint = buildHeroLocationHint(
    meta,
    displayNameHint || curCityDisplay || curCity,
    data?.resolved_station_name || data?.city?.name || ''
  );
  if (heroHint) curHeroQueryHint = heroHint;
  else if (hintedCity) curHeroQueryHint = hintedCity;
...
function animateGaugeValue(targetAqi, cat) {
  const gaugeValueEl = $('gaugeValue');
  if (!gaugeValueEl) return;
  const safeTarget = Math.max(0, Math.round(Number(targetAqi) || 0));
  const currentText = parseAqiNumber(gaugeValueEl.textContent);
  const startValue = Number.isFinite(gaugeValueAnimatedValue)
    ? gaugeValueAnimatedValue
    : (Number.isFinite(currentText) ? currentText : safeTarget);
  if (gaugeValueAnimationFrame) {
    cancelAnimationFrame(gaugeValueAnimationFrame);
    gaugeValueAnimationFrame = 0;
  }
  const duration = Math.abs(safeTarget - startValue) > 80 ? 1100 : 780;
  ...
  const step = now => {
    ...
    gaugeValueAnimatedValue = nextValue;
    gaugeValueEl.textContent = String(Math.round(nextValue));
    gaugeValueEl.style.color = cat.color;
    if (progress < 1) {
      gaugeValueAnimationFrame = requestAnimationFrame(step);
      return;
    }
    gaugeValueAnimatedValue = safeTarget;
    gaugeValueEl.textContent = String(safeTarget);
    gaugeValueAnimationFrame = 0;
  };
...
function updateGaugeLiveOrbit(aqi, cat) {
  const safeAqi = Math.max(0, Math.min(500, Number(aqi) || 0));
  const orbit = $('gaugeLiveOrbit');
  if (orbit) {
    const speedSec = Math.max(4.6, 10.5 - (safeAqi / 500) * 5.2);
    orbit.style.borderColor = `${cat.color}33`;
    orbit.style.animationDuration = `${speedSec.toFixed(2)}s`;
    orbit.style.boxShadow = `inset 0 0 0 1px ${cat.color}12`;
  }
  document.querySelectorAll('.gauge-live-orbit-dot').forEach((dot, idx) => {
    dot.style.background = cat.color;
    dot.style.boxShadow = `0 0 0 ${idx === 0 ? 5 : 4}px ${cat.color}14, 0 0 18px ${cat.color}55`;
  });
  const livePill = $('gaugeLivePill');
  if (livePill) {
    livePill.style.borderColor = `${cat.color}33`;
    livePill.style.boxShadow = `0 8px 18px ${cat.color}18`;
  }
  const liveDot = document.querySelector('.gauge-live-dot');
  if (liveDot) liveDot.style.background = cat.color;
}
...
function updateHeroUI(cityName, country, aqi, cat, desc, reqSeq = null) {
  ...
  updateCinematicHero({
    ...
    timeIso: curTimeIso,
    bgHint: curHeroQueryHint || 'city skyline',
  }, reqSeq);
}
...
function updatePollutants(data, city, dominant) {
  ...
  grid.innerHTML = Object.entries(POLL_CFG).map(([key, cfg]) => {
    ...
    return `<div class="p-card fade-in">
      ...
      <div class="pc-graph" data-graph="${key}" aria-hidden="true">
        <div class="pc-graph-loading"></div>
      </div>
  ...
  loadPollutantGraphs(city || currentLiveQuery || curCityDisplay || curCity).catch(() => { });
}
...
function buildForecastRequestParams(cityQuery, curAqi, displayName = '') {
  const params = new URLSearchParams();
  const rawCity = String(cityQuery || '').trim();
  if (rawCity) params.set('city', rawCity);
  const label = normalizeDisplayName(displayName || getForecastCityName(rawCity));
  if (label) params.set('label', label);

  const aqiNum = Number(curAqi);
  if (Number.isFinite(aqiNum)) params.set('aqi', String(Math.round(aqiNum)));

  const iaqi = curLiveData?.iaqi || {};
  const addIfFinite = (key, value) => {
    const num = Number(value);
    if (Number.isFinite(num)) params.set(key, String(num));
  };

  addIfFinite('pm25', iaqi.pm25?.v);
  addIfFinite('pm10', iaqi.pm10?.v);
  addIfFinite('no2', iaqi.no2?.v);
  addIfFinite('so2', iaqi.so2?.v);
  addIfFinite('o3', iaqi.o3?.v);
  addIfFinite('co', iaqi.co?.v);
  addIfFinite('temperature', iaqi.t?.v);
  addIfFinite('humidity', iaqi.h?.v);
  addIfFinite('wind_speed', iaqi.w?.v);
...
async function renderLstmForecast(cityQuery, curAqi) {
  if (typeof Chart === 'undefined') {
    console.warn('Chart.js not loaded yet, skipping LSTM forecast chart');
    return;
  }
  ...
  let fc = [];
  try {
    const params = buildForecastRequestParams(cityQuery, curAqi);
    const j = await fetchJsonNoCache(`/api/predict/7day?${params.toString()}`);
  ...
  if (fc.length) {
    fc.forEach((d, i) => {
      labels.push(String(d.day_name || '').substring(0, 3) || formatForecastLabel('', i));
...
function getForecastCityName(rawCity = '', fallbackCity = '') {
  const raw = String(rawCity || '').trim();
  const fallback = normalizeDisplayName(
    fallbackCity ||
    currentLiveLocation?.label ||
    currentUserCity ||
    curCityDisplay ||
    curCity ||
    'Delhi'
  );
  if (!raw || raw.startsWith('@') || raw.startsWith('geo:')) return fallback || 'Delhi';
  const parsed = parseMapStationLocation(raw, fallback || raw);
  return normalizeDisplayName(parsed.city || raw) || fallback || 'Delhi';
}

async function renderForecast(forecast, curAqi) {
  const cityName = getForecastCityName(curCityDisplay || currentUserCity || curCity, curCityDisplay || currentUserCity || curCity);
  return renderLstmForecast(cityName, Number.isFinite(curAqi) ? curAqi : getDisplayedAqiFallback());
}

async function loadForecastCoach(cityQuery = '', displayName = '', curAqi = null) {
  const coachEl = $('forecastCoach');
  if (!coachEl) return;
  const thisSeq = ++forecastCoachLoadSeq;
  const label = normalizeDisplayName(displayName || getForecastCityName(cityQuery));
  coachEl.textContent = `AI Coach: Generating a short guidance note for ${label || 'your location'}…`;

  try {
    const iaqi = curLiveData?.iaqi || {};
    const aqiNum = Number.isFinite(Number(curAqi)) ? Number(curAqi) : Number(curLiveData?.aqi) || 50;
    const params = new URLSearchParams({
      city: label || 'Your location',
      aqi: String(Math.round(aqiNum)),
      dominant: String(curLiveData?.dominentpol || getDominantPollutantFromIaqi(iaqi) || 'pm25'),
      temp: Number.isFinite(Number(iaqi?.t?.v)) ? String(Number(iaqi.t.v)) : '',
      humidity: Number.isFinite(Number(iaqi?.h?.v)) ? String(Number(iaqi.h.v)) : '',
      wind: Number.isFinite(Number(iaqi?.w?.v)) ? String(Number(iaqi.w.v)) : '',
      time_iso: curLiveData?.time?.iso || curTimeIso || '',
    });
    const resp = await fetchJsonNoCache(`/api/nlp/advice?${params.toString()}`);
    if (thisSeq !== forecastCoachLoadSeq) return;
    const advice = resp?.data || resp?.advice || resp;
    const summary = String(advice?.summary || advice?.assistant_reply || '').replace(/\s+/g, ' ').trim();
    coachEl.textContent = summary
      ? `AI Coach: ${summary}`
      : `AI Coach: Keep following the live AQI trend for ${label || 'your location'}.`;
  } catch {
    if (thisSeq !== forecastCoachLoadSeq) return;
    coachEl.textContent = `AI Coach: Keep following the live AQI trend for ${displayName || 'your location'}.`;
  }
}
...
function getRequestedTrendCity(explicitCity = undefined) {
  ...
  if (selectionState.mode === 'locality' && selectionState.query) return selectionState.query;
  if (currentLiveQuery) return currentLiveQuery;
  if (currentUserCity) return currentUserCity;
  const sel = $('trendCitySelect');
  const picked = String(sel?.value || '').trim();
  if (picked) return picked;
  return curCityDisplay || curCity;
}
...
async function loadTrend(city = undefined) {
  try {
    ...
    if (typeof Chart === 'undefined') {
      console.warn('Chart.js not loaded yet, skipping trend chart');
      return;
    }
    ...
    populateCitySelect(currentLiveLocation?.label || requestedCity);
  } catch { }
}
...
async function loadDonut() {
  try {
    // Check if Chart.js is loaded
    if (typeof Chart === 'undefined') {
      console.warn('Chart.js not loaded yet, skipping donut chart');
      return;
    }
...
/* ── Lungs Update ───────────────────────────────────────── */
async function updateLungs() {
  try {
    const el = $('aiLungEffects');
    if (!el || !curLiveData) return;
    const aqi = resolveLiveAqi(curLiveData, 50) || 50;
    const pm25 = Number(curLiveData?.pollutants?.pm25 ?? curLiveData?.iaqi?.pm25?.v ?? 0) || 0;
    let tone = 'Calm';
    let text = 'Lungs functioning optimally with clean air.';
    let action = 'Keep up the normal routine';
    if (aqi <= 50) {
      tone = 'Calm';
      text = 'Lungs functioning optimally with clean air.';
      action = 'Keep the routine steady';
    } else if (aqi <= 100) {
      tone = 'Watch';
      text = 'Moderate air quality may cause mild irritation.';
      action = 'Reduce heavy outdoor exertion';
    } else if (aqi <= 150) {
      tone = 'Caution';
      text = 'Poor air quality is increasing lung stress.';
      action = 'Prefer a mask outdoors';
    } else if (aqi <= 200) {
      tone = 'High';
      text = 'Unhealthy air is raising breathing strain.';
      action = 'Limit outdoor exposure';
    } else {
      tone = 'Critical';
      text = 'Hazardous conditions are severely impacting lung health.';
      action = 'Stay indoors if possible';
    }
    el.innerHTML = `
      <div class="lung-summary-head">
        <span class="lung-tone">${escapeHtml(tone)}</span>
        <span class="lung-aqi-chip">${Math.round(aqi)} AQI</span>
      </div>
      <p>${escapeHtml(text)}</p>
      <div class="lung-pill-row">
        <span class="lung-pill"><i class="fa-solid fa-wind"></i> PM2.5 ${pm25 ? pm25.toFixed(1) : '—'}</span>
        <span class="lung-pill"><i class="fa-solid fa-heart-pulse"></i> ${escapeHtml(action)}</span>
      </div>`;
  } catch { }
}
...
async function loadForecast(city = '') {
  try {
    ...
    const forecastQuery = String(currentLiveQuery || city || selectionState.query || currentUserCity || curCityDisplay || curCity || '').trim();
    const displayCity = getForecastCityName(forecastQuery, currentLiveLocation?.label || curCityDisplay || currentUserCity || curCity);
    const currentAqi = Number.isFinite(Number(curLiveData?.aqi)) ? Number(curLiveData.aqi) : 50;
    if (metaEl) {
      metaEl.textContent = `AI projection for ${displayCity} based on the current live AQI and weather conditions.`;
    }
    const coachEl = $('forecastCoach');
    if (coachEl) coachEl.textContent = `AI Coach: Reading the live trend for ${displayCity}…`;
    container.innerHTML = '<div class="forecast-loading">Loading forecast...</div>';

    const params = buildForecastRequestParams(forecastQuery, currentAqi, displayCity);
    const url = `/api/predict/7day?${params.toString()}`;
    const d = await fetchJsonNoCache(url);
    if (!d.forecast || !Array.isArray(d.forecast)) {
      container.innerHTML = '<div class="forecast-loading">Forecast unavailable</div>';
      return;
    }

    if (metaEl) {
      const summary = String(d.summary || '').trim();
      metaEl.textContent = summary || `AI projection for ${displayCity} based on the current live AQI and weather conditions.`;
    }

    const items = d.forecast.map((f, idx) => {
      const aqi = Number(f.predicted_aqi) || 0;
      const pct = Math.max(0, Math.min(100, Math.round((aqi / 500) * 100)));
      const prev = idx > 0 ? Number(d.forecast[idx - 1]?.predicted_aqi || aqi) : aqi;
      const delta = aqi - prev;
      const deltaText = idx === 0 ? 'Start' : (delta === 0 ? 'Steady' : delta > 0 ? `+${delta}` : `${delta}`);
      return `
      <article class="forecast-item">
        <div class="forecast-day">${f.day_name}</div>
        <div class="forecast-date">${f.date}</div>
        <div class="forecast-aqi" style="color: ${f.color}">${aqi}</div>
        <div class="forecast-level" style="background: ${f.color}20; color: ${f.color}">${f.level}</div>
        <div class="forecast-rail" aria-hidden="true"><span style="width:${pct}%;background:${f.color}"></span></div>
        <div class="forecast-delta">${deltaText}</div>
      </article>`;
    }).join('');
    container.innerHTML = items;
    loadForecastCoach(forecastQuery, displayCity, currentAqi).catch(() => { });
  } catch (e) {
    console.error('Forecast load error:', e);
    const container = $('forecastContainer');
    if (container) container.innerHTML = '<div class="forecast-loading">Error loading forecast</div>';
    const coachEl = $('forecastCoach');
    if (coachEl) coachEl.textContent = 'AI Coach: Forecast guidance is temporarily unavailable.';
  }
}

function buildSparklineSvg(values, color, id = 'spark') {
  const width = 120;
  const height = 44;
  const pad = 6;
  ...
  const safeSeries = series.length >= 2
    ? series.slice(-24)
    : (series.length === 1 ? [series[0], series[0], series[0], series[0]] : [0, 0, 0, 0]);
  const max = Math.max(...safeSeries);
  const min = Math.min(...safeSeries);
  const range = Math.max(1, max - min);
  const flatSeries = max === min;
  const pointCount = Math.max(2, safeSeries.length);
  const points = safeSeries.map((v, idx) => {
    const x = pad + (idx / Math.max(1, pointCount - 1)) * (width - pad * 2);
    const y = flatSeries
      ? height / 2
  ...
  const gradientId = `spark-${String(id || 'spark').replace(/[^a-z0-9_-]/gi, '')}`;
  return `
    <svg class="pc-graph-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="Pollutant trend chart">
      <defs>
      ...
      <circle cx="${last.x.toFixed(1)}" cy="${last.y.toFixed(1)}" r="2.8" fill="${color}"></circle>
    </svg>`;
}

async function loadPollutantGraphs(cityQuery = '') {
  const grid = $('pollutantsGrid');
  if (!grid) return;
  const thisSeq = ++pollutantGraphLoadSeq;
  const query = String(cityQuery || currentLiveQuery || selectionState.query || currentUserCity || curCityDisplay || curCity || '').trim();
  const cards = Array.from(grid.querySelectorAll('.p-card'));
  if (!cards.length) return;

  const liveSeed = extractLivePollutants(curLiveData || {});
  const renderFallback = () => {
    cards.forEach(card => {
      const key = String(card.querySelector('.pc-graph')?.dataset.graph || '').trim();
      const cfg = POLL_CFG[key] || {};
      const current = Number(liveSeed[key]);
      const values = Number.isFinite(current)
        ? [current * 0.88, current * 0.95, current, current * 1.06, current * 1.12]
        : [0, 0, 0, 0];
      const target = card.querySelector('.pc-graph');
      if (target) target.innerHTML = buildSparklineSvg(values, cfg.color || '#2b7fff', key);
    });
  };
  ...
  try {
    const params = new URLSearchParams({ hours: '24', fresh: '1' });
    params.set('city', query);
    const d = await fetchJsonNoCache(`/api/historical?${params.toString()}`);
    if (thisSeq !== pollutantGraphLoadSeq) return;

    const seriesMap = {
      pm25: Array.isArray(d?.pm25) ? d.pm25 : [],
      pm10: Array.isArray(d?.pm10) ? d.pm10 : [],
      no2: Array.isArray(d?.no2) ? d.no2 : [],
      so2: Array.isArray(d?.so2) ? d.so2 : [],
      o3: Array.isArray(d?.o3) ? d.o3 : [],
    ...
    cards.forEach(card => {
      ...
      const target = card.querySelector('.pc-graph');
      if (!target) return;
      const values = seriesMap[key];
      const hasSeries = Array.isArray(values) && values.some(v => Number.isFinite(Number(v)));
      const sourceValues = hasSeries
        ? values
        : (Number.isFinite(Number(liveSeed[key]))
          ? [liveSeed[key] * 0.88, liveSeed[key] * 0.95, liveSeed[key], liveSeed[key] * 1.06, liveSeed[key] * 1.12]
          : [0, 0, 0, 0]);
      target.innerHTML = buildSparklineSvg(sourceValues, cfg.color || '#2b7fff', key);
    });
  } catch {
    if (thisSeq !== pollutantGraphLoadSeq) return;
    renderFallback();
...
function initForecast() {
  // The forecast band is loaded from the main boot / city refresh path.
}
...
async function loadRanking() {
  try {
    ...
    if (selectionState.mode === 'locality' && selectionState.query) {
      const d = await fetchJsonNoCache(buildNearbyAreaRequestUrl(selectionState.query, 12, 24));
      if (Array.isArray(d?.areas) && d.areas.length) {
        ...
        if (rankingMeta) {
          const focusLabel = currentLiveLocation?.label || selectionState.locality || curCityDisplay || 'your selected place';
          rankingMeta.textContent = `Nearby locations around ${focusLabel} sorted by live AQI.`;
        }
...
async function refreshLiveAqiOnly() {
  ...
  try {
    const j = await fetchLivePayloadForQuery(activeCity, {
      label: currentLiveLocation?.label || displayHint,
      accuracy: currentLiveLocation?.accuracy,
      fresh: true,
    });
    if (isStaleReq(reqSeq)) return;
    ...
    curLiveData = resolvedStationMatchesQuery(activeCity)
      ? applyResolvedStationToLiveData(j.data, currentResolvedStation, displayHint, {
        lat: currentLiveLocation?.lat,
        lng: currentLiveLocation?.lng,
      })
      : j.data;
    ...
    rememberLiveSnapshot([activeCity, displayHint, curHeroQueryHint, curLiveData?.resolved_station_name, curLiveData?.city?.name], curLiveData);
    ...
    loadAreaAqiList(activeCity, reqSeq).catch(() => { });
    loadForecast().catch(() => { });
    loadDonut();
    loadNlpAdvice(curLiveData, reqSeq);
    loadRanking();
  ...
  (async function init() {
    ...
    try {
      ...
      await Promise.all([
        ...
        withTimeout(loadForecast(), 5000),
      ]);

      // initialize map and forecast
      try { initMap(); } catch (e) { console.error('initMap error', e); }
      try { initForecast(); } catch (e) { console.error('initForecast error', e); }
    } catch (e) {
