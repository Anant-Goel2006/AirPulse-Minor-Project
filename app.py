...
APP_USER_AGENT = f"AirPulse/{APP_BUILD_TS}"
...
def looks_like_email(value):
    ...
    return bool(re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", txt))


SMTP_LOGIN_USER = str(
    os.getenv("SMTP_LOGIN_USER")
    or (SMTP_USER if looks_like_email(SMTP_USER) else SMTP_FROM_EMAIL)
    or ""
).strip()
SMTP_SENDER_NAME = str(
    os.getenv("SMTP_SENDER_NAME")
    or ("" if looks_like_email(SMTP_USER) else SMTP_USER)
    or ""
).strip()
SMTP_LOGIN_PASSWORD = str(SMTP_PASSWORD or "").strip()
if "gmail" in SMTP_HOST.lower():
    SMTP_LOGIN_PASSWORD = re.sub(r"\s+", "", SMTP_LOGIN_PASSWORD)


def format_smtp_from_header():
    if SMTP_SENDER_NAME and SMTP_FROM_EMAIL:
        return formataddr((SMTP_SENDER_NAME, SMTP_FROM_EMAIL))
    return SMTP_FROM_EMAIL or SMTP_SENDER_NAME
...
def waqi_get_json(path, params=None, timeout=8, log_label="WAQI"):
    ...
    try:
        resp = HTTP_SESSION.get(endpoint, params=query, timeout=timeout)
        try:
            payload = resp.json()
...
def _weighted_mean_pairs(pairs, digits=1):
    total = 0.0
    total_weight = 0.0
    for value, weight in pairs:
        v = parse_aqi_value(value)
        try:
            w = float(weight)
        except Exception:
            w = None
    ...
    if digits is None:
        return float(out)
    return round(float(out), int(digits))


def _blend_location_weight(distance_km):
    try:
        dist = max(0.0, float(distance_km))
    except Exception:
        dist = 9999.0
    return 1.0 / pow(max(0.35, dist + 0.35), 1.35)


def _infer_location_identity(rows, fallback_label=""):
    candidates = {}
    source_rows = rows or []
    for row in source_rows[:10]:
        ...
        if not city:
            loc = location_from_station_name(row.get("station_name") or "", fallback=fallback_label)
            area = area or str(loc.get("area") or "").strip()
            city = city or str(loc.get("city") or "").strip()
            country = country or str(loc.get("country") or "").strip()
        if not city and not area:
            continue
        key = (
            normalize_lookup_key(area),
            normalize_lookup_key(city),
        ...
        score = _blend_location_weight(row.get("distance_km"))
        if area and normalize_lookup_key(area) != normalize_lookup_key(city):
            score += 0.18
        entry["score"] += score

    if not candidates:
        loc = location_from_station_name(fallback_label or "", fallback=fallback_label)
        city = str(loc.get("city") or display_label_from_query(fallback_label) or "Your area").strip()
        area = str(loc.get("area") or "").strip()
        country = str(loc.get("country") or "").strip()
    else:
        best = max(candidates.values(), key=lambda item: item.get("score", 0.0))
        area = str(best.get("area") or "").strip()
        city = str(best.get("city") or "").strip()
        country = str(best.get("country") or "").strip()

    if not city:
        city = display_label_from_query(fallback_label) or "Your area"
    if area and normalize_lookup_key(area) != normalize_lookup_key(city):
        label = f"{area}, {city}"
    else:
        label = city
...
def _extract_station_detail_from_feed(feed_payload, fallback_row):
    row = fallback_row or {}
    detail = {
        "payload": feed_payload if is_valid_feed_payload(feed_payload) else None,
        "aqi": parse_aqi_value(row.get("aqi")),
        "station_name": str(row.get("station_name") or "").strip(),
        "lat": parse_aqi_value(row.get("lat")),
        "lng": parse_aqi_value(row.get("lng")),
        "distance_km": safe_float(row.get("distance_km"), 9999.0),
        "area": str(row.get("area") or "").strip(),
        "city": str(row.get("city") or "").strip(),
        "country": str(row.get("country") or "").strip(),
        "pollutants": {},
    ...
    if not is_valid_feed_payload(feed_payload):
        return detail

    data = feed_payload.get("data") or {}
    city_meta = data.get("city") if isinstance(data.get("city"), dict) else {}
    station_name = str(city_meta.get("name") or detail["station_name"]).strip()
    detail["aqi"] = parse_aqi_value(data.get("aqi")) or detail["aqi"]
    detail["station_name"] = station_name
    detail["dominentpol"] = str(data.get("dominentpol") or "").strip()
    detail["time"] = data.get("time") if isinstance(data.get("time"), dict) else {}

    geo = city_meta.get("geo") if isinstance(city_meta, dict) else None
    if isinstance(geo, (list, tuple)) and len(geo) >= 2:
        detail["lat"] = parse_aqi_value(geo[0]) or detail["lat"]
        detail["lng"] = parse_aqi_value(geo[1]) or detail["lng"]

    loc = location_from_station_name(station_name, fallback=detail["city"] or "")
    detail["area"] = str(loc.get("area") or detail["area"] or "").strip()
    detail["city"] = str(loc.get("city") or detail["city"] or "").strip()
    detail["country"] = str(loc.get("country") or detail["country"] or "").strip()

    iaqi = data.get("iaqi") if isinstance(data.get("iaqi"), dict) else {}
    for key in ("pm25", "pm10", "no2", "so2", "o3", "co"):
        detail["pollutants"][key] = _extract_iaqi_value(iaqi, key)
    detail["weather"] = {
        "temperature": _extract_iaqi_value(iaqi, "t"),
        "humidity": _extract_iaqi_value(iaqi, "h"),
        "wind_speed": _extract_iaqi_value(iaqi, "w"),
    }
    return detail


def _build_synthetic_iaqi(base_data, pollutant_values, weather_values):
    iaqi = _json_clone((base_data or {}).get("iaqi") or {}) or {}
    for key, value in (pollutant_values or {}).items():
        if parse_aqi_value(value) is not None:
            iaqi[key] = {"v": round(float(value), 2)}
    for field, iaqi_key in (("temperature", "t"), ("humidity", "h"), ("wind_speed", "w")):
        value = (weather_values or {}).get(field)
        if parse_aqi_value(value) is not None:
            iaqi[iaqi_key] = {"v": round(float(value), 2)}
    return iaqi


def _attach_live_location_context(
    feed_payload,
    center_lat,
    center_lng,
    location_meta,
    nearest=None,
    match_mode="nearby_station",
    estimated=False,
    stations_considered=1,
    radius_km=None,
):
    out = _json_clone(feed_payload) or {}
    ...
    city_meta["name"] = str(location_meta.get("label") or city_meta.get("name") or "Your area").strip()
    city_meta["geo"] = [round(float(center_lat), 6), round(float(center_lng), 6)]

    data["user_location"] = {
        "label": str(location_meta.get("label") or city_meta.get("name") or "Your area").strip(),
        "area": str(location_meta.get("area") or "").strip(),
        "city": str(location_meta.get("city") or "").strip(),
        "country": str(location_meta.get("country") or "").strip(),
        "lat": round(float(center_lat), 6),
        "lng": round(float(center_lng), 6),
        "estimated": bool(estimated),
        "match_mode": str(match_mode or "").strip(),
        "stations_considered": max(1, int(stations_considered or 1)),
        "radius_km": round(float(radius_km), 3) if parse_aqi_value(radius_km) is not None else None,
    }

    if isinstance(nearest, dict):
        nearest_meta = {
            "uid": nearest.get("uid"),
            "station_name": str(nearest.get("station_name") or "").strip(),
            "lat": round(float(nearest.get("lat")), 6) if parse_aqi_value(nearest.get("lat")) is not None else None,
            "lng": round(float(nearest.get("lng")), 6) if parse_aqi_value(nearest.get("lng")) is not None else None,
            "distance_km": round(float(nearest.get("distance_km") or 0.0), 3),
            "area": str(nearest.get("area") or "").strip(),
            "city": str(nearest.get("city") or "").strip(),
            "country": str(nearest.get("country") or "").strip(),
        }
        data["resolved_station"] = nearest_meta
        if nearest_meta["station_name"]:
            data["resolved_station_name"] = nearest_meta["station_name"]
        out["nearest"] = nearest_meta
...
def _build_blended_live_payload(center_lat, center_lng, rows, fallback_label="", radius_limit=20.0):
    ranked = sorted(
        rows or [],
        key=lambda row: (
            safe_float(row.get("distance_km"), 9999.0),
            safe_float(row.get("aqi"), 9999.0),
        ),
    )
    if len(ranked) < 2:
        return None

    nearest_dist = safe_float(ranked[0].get("distance_km"), 9999.0)
    primary_radius = min(max(2.5, nearest_dist * 3.2), min(max(float(radius_limit or 20.0), 2.5), 8.0))
    candidates = [row for row in ranked if safe_float(row.get("distance_km"), 9999.0) <= primary_radius][:5]
    if len(candidates) < 2:
        fallback_radius = min(max(5.0, nearest_dist * 4.2), min(max(float(radius_limit or 20.0), 5.0), 14.0))
        candidates = [row for row in ranked if safe_float(row.get("distance_km"), 9999.0) <= fallback_radius][:5]
    if len(candidates) < 2:
        return None
    ...
    def fetch_detail(row):
        uid = row.get("uid")
        if uid is None:
            return _extract_station_detail_from_feed(None, row)
        payload, _ = fetch_feed(f"@{uid}", timeout=6)
        return _extract_station_detail_from_feed(payload, row)

    details = []
    with ThreadPoolExecutor(max_workers=max(2, min(len(candidates), 4))) as executor:
        ...
        for fut in as_completed(futures):
            try:
                ...
                detail = None
            if detail and detail.get("aqi") is not None:
                details.append(detail)

    details.sort(key=lambda item: item.get("distance_km", 9999.0))
    if len(details) < 2:
        return None

    for detail in details:
        detail["weight"] = _blend_location_weight(detail.get("distance_km"))

    blended_aqi = _weighted_mean_pairs(
        [(detail.get("aqi"), detail.get("weight")) for detail in details],
        digits=0,
    )
    if blended_aqi is None:
        return None

    pollutants = {}
    for key in ("pm25", "pm10", "no2", "so2", "o3", "co"):
        pollutants[key] = _weighted_mean_pairs(
            [(detail["pollutants"].get(key), detail.get("weight")) for detail in details],
            digits=2,
        )

    weather = {}
    for key in ("temperature", "humidity", "wind_speed"):
        weather[key] = _weighted_mean_pairs(
            [(detail["weather"].get(key), detail.get("weight")) for detail in details],
            digits=2,
        )

    dominant = ""
    pollutant_rank = {k: v for k, v in pollutants.items() if v is not None}
    if pollutant_rank:
        dominant = max(pollutant_rank.items(), key=lambda item: item[1])[0]

    base_detail = next((detail for detail in details if detail.get("payload")), details[0])
    base_payload = base_detail.get("payload") or {"status": "ok", "data": {}}
    location_meta = _infer_location_identity(details, fallback_label=fallback_label)
    out = _attach_live_location_context(
        base_payload,
        center_lat=center_lat,
        center_lng=center_lng,
        location_meta=location_meta,
        nearest=ranked[0],
        match_mode="nearby_average",
        estimated=True,
        stations_considered=len(details),
        radius_km=max(detail.get("distance_km", 0.0) for detail in details),
    )
    ...
    base_data = (base_payload or {}).get("data") if isinstance(base_payload, dict) else {}
    data["aqi"] = int(round(float(blended_aqi)))
    data["dominentpol"] = dominant or str(base_data.get("dominentpol") or "").strip()
    data["iaqi"] = _build_synthetic_iaqi(base_data, pollutants, weather)

    time_meta = _json_clone(base_data.get("time") or {}) if isinstance(base_data, dict) else {}
    if not isinstance(time_meta, dict):
        time_meta = {}
    if not str(time_meta.get("s") or "").strip():
        time_meta["s"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    data["time"] = time_meta
    return out
...
@app.route("/api/live/areas")
def live_area_list_coords():
    """Area-wise live AQI list around explicit coordinates."""
    if not WAQI_TOKEN:
        return waqi_token_missing_response("status")

    try:
        center_lat = float(request.args.get("lat", ""))
        center_lng = float(request.args.get("lng", ""))
    except Exception:
    ...
    label = normalize_query_text(request.args.get("label") or request.args.get("query") or "Your location")
    ...
    if not rows:
        ...
        if code == 200 and is_valid_feed_payload(payload):
            base = build_current_aqi_from_live_payload(payload, requested_city=label)
            nearest = payload.get("nearest") if isinstance(payload, dict) else {}
            data = payload.get("data") if isinstance(payload, dict) else {}
            city_meta = data.get("city") if isinstance(data, dict) else {}
            geo = city_meta.get("geo") if isinstance(city_meta, dict) else None
            lat = center_lat
            lng = center_lng
            if isinstance(geo, (list, tuple)) and len(geo) >= 2:
            ...
            if isinstance(base, dict):
                rows = [{
                    "uid": data.get("idx"),
                    "aqi": int(round(float(base.get("aqi") or 0.0))),
                    "station_name": str(base.get("station_name") or label).strip(),
                    "area": str(base.get("area") or "").strip(),
                    "city": str(base.get("city") or label).strip(),
                    "country": str(base.get("country") or "").strip(),
                    "lat": round(float(lat), 6),
                    "lng": round(float(lng), 6),
                    "distance_km": float(nearest.get("distance_km") or 0.0),
                }]
...
def resolve_nearest_live_station(lat, lng, radius_limit=20.0, label_hint=""):
    ...
    rows = collect_live_area_rows(
        center_lat=lat,
        center_lng=lng,
        fallback_city=label_hint,
        radius_limit=max(6.0, radius_limit),
        max_rows=80,
    )
    ranked = sorted(
        rows or [],
    ...
    if nearest and nearest.get("uid") is not None:
        nearest_dist = safe_float(nearest.get("distance_km"), 9999.0)
        if nearest_dist <= 1.5:
            feed_payload, _ = fetch_feed(f"@{nearest['uid']}")
            if is_valid_feed_payload(feed_payload):
                location_meta = _infer_location_identity(ranked, fallback_label=label_hint)
                out = _attach_live_location_context(
                    feed_payload,
                    center_lat=lat,
                    center_lng=lng,
    ...
    blended = _build_blended_live_payload(
        center_lat=lat,
        center_lng=lng,
        rows=ranked,
        fallback_label=label_hint,
        radius_limit=radius_limit,
    )
    if isinstance(blended, dict) and str(blended.get("status", "")).lower() == "ok":
        matched_uid = nearest.get("uid") if isinstance(nearest, dict) else None
        blended = with_resolved_meta(
            blended,
            f"{lat},{lng}",
            f"{lat},{lng}",
            "nearby_blend",
    ...
    if nearest and nearest.get("uid") is not None:
        ...
        if is_valid_feed_payload(feed_payload):
            ...
            out = _attach_live_location_context(
                ...
                location_meta=location_meta,
                nearest=nearest,
                match_mode="nearest_station_fallback",
                estimated=True,
                stations_considered=1,
                radius_km=nearest.get("distance_km"),
            )
            out = with_resolved_meta(
                out,
                f"{lat},{lng}",
...
def live_nearby():
    ...
    payload, code = resolve_nearest_live_station(
        ...
        label_hint=normalize_query_text(request.args.get("label") or ""),
    )
...
def location_bootstrap():
    ...
    if WAQI_TOKEN:
        payload, payload_code = resolve_nearest_live_station(
            ...
            radius_limit=request.args.get("radius_km"),
            label_hint=location.get("city") or "",
        )
        if payload_code == 200:
...
def build_live_row_from_payload(live_payload, requested_city=""):
    ...
    return {
        "query": normalize_query_text(requested_city),
        "city_key": city_key,
...
def _snapshot_key_from_row(row):
    ...
    return normalize_query_text(
        row.get("query") or row.get("city_key") or row.get("city") or ""
    ).lower().strip()
...
def _city_key_match(requested_key, candidate_key):
    ...
    if req == cand:
        return True
    if req.startswith("geo:") or cand.startswith("geo:") or req.startswith("@") or cand.startswith("@"):
        return False
...
# ── City Image Proxy (Pexels → Getty → Unsplash fallback) ────────────────
...
def build_city_image_fallback_url(seed_text):
    cleaned = re.sub(r"[^a-z0-9 ]", "", str(seed_text or "").strip().lower()).strip()
    cleaned = re.sub(r"\s+", " ", cleaned)
    if not cleaned:
...
def city_image_proxy(city):
    """Return a live image URL for a city. Tries Pexels/Gettty first, falls back to Unsplash."""
    try:
        city_key = re.sub(r"[^a-z0-9 ]", "", str(city or "").strip().lower()).strip()
        if not city_key:
            return jsonify({"error": "Empty city"}), 400

        search_queries = build_city_image_queries(city_key)
        ...
        # Try Pexels API first because it aligns with the env the project uses.
        if PEXELS_API_KEY:
            try:
        ...
        if not image_url:
            image_url = build_city_image_fallback_url(city_key)
            source = "unsplash"
...
def send_alert_email(to_email, subject, text_body):
    ...
    msg = EmailMessage()
    msg["Subject"] = subject
    msg["From"] = format_smtp_from_header()
    msg["To"] = to_email
    ...
    if SMTP_USE_TLS:
        ...
        try:
            ...
            raise RuntimeError(
                "SMTP login failed. For Gmail use your Gmail address as the login user "
                "and a 16-character App Password, not your normal account password."
            ) from exc
    else:
        try:
...
def _collect_forecast_history(city_query="", lookback_hours=168):
    requested_key = normalize_query_text(city_query).lower().strip()
    cutoff_epoch = time.time() - max(1, int(lookback_hours)) * 3600
    entries = []
    ...
    entries.sort(key=lambda entry: float(entry.get("timestamp_epoch") or 0.0))
    return entries


def _generate_7day_forecast_series(base_aqi, history_entries, weather=None, days=7):
    base_aqi = float(base_aqi if base_aqi is not None else 50.0)
    days = max(1, min(int(days or 7), 7))

    history_vals = [
        float(val)
        for val in (_to_float_or_none(entry.get("aqi")) for entry in history_entries or [])
        if val is not None
    ]
    recent_vals = history_vals[-min(len(history_vals), 24):]

    if len(recent_vals) >= 2:
        try:
            x = np.arange(len(recent_vals), dtype=float)
            y = np.array(recent_vals, dtype=float)
            slope_per_hour = float(np.polyfit(x, y, 1)[0])
            trend_per_day = max(min(slope_per_hour * 24.0, 28.0), -28.0)
        except Exception:
            trend_per_day = (95.0 - base_aqi) * 0.08
        recent_mean = float(np.mean(recent_vals))
        volatility = float(np.std(recent_vals))
    else:
        recent_mean = base_aqi
        volatility = 0.0
        trend_per_day = (95.0 - base_aqi) * 0.08

    weather_bias = 0.0
    if isinstance(weather, dict):
        humidity = _to_float_or_none(weather.get("humidity"))
        wind_speed = _to_float_or_none(weather.get("wind_speed"))
        temperature = _to_float_or_none(weather.get("temperature"))

        if humidity is not None:
            weather_bias += (humidity - 55.0) * 0.10
        if wind_speed is not None:
            weather_bias += (2.5 - wind_speed) * 4.0
        if temperature is not None:
            weather_bias += max(-4.0, min(4.0, (temperature - 28.0) * 0.12))
    ...
    for offset in range(1, days + 1):
        forecast_dt = start_dt + timedelta(days=offset)
        mean_reversion = recent_mean + (base_aqi - recent_mean) * math.exp(-offset / 3.5)
        trend_target = base_aqi + trend_per_day * offset
        smooth_target = (0.55 * trend_target) + (0.35 * mean_reversion) + (0.10 * (base_aqi + weather_bias))
        seasonal = math.sin((forecast_dt.weekday() + offset) * (math.pi / 3.5)) * min(8.0, 2.5 + (volatility * 0.15))
        predicted = (0.70 * prev_value) + (0.30 * (smooth_target + seasonal))
        if volatility > 0:
            predicted += math.sin((offset + len(recent_vals)) * 1.35) * min(6.0, volatility * 0.18)
        predicted = max(0.0, min(500.0, predicted))
        prev_value = predicted

        cat = get_category(int(round(predicted)))
        out.append({
            "day_name": forecast_dt.strftime("%a"),
            "date": forecast_dt.strftime("%d %b"),
            "predicted_aqi": int(round(predicted)),
            "level": cat["level"],
            "color": cat["color"],
        })
...
def _summarize_7day_forecast(city_label, forecast_rows):
    if not forecast_rows:
        return f"AI projection for {city_label} is temporarily unavailable."
    values = [float(row.get("predicted_aqi") or 0) for row in forecast_rows]
    first = int(round(values[0]))
    last = int(round(values[-1]))
    spread = int(round(max(values) - min(values))) if values else 0
    if last <= first - 5:
        direction = "improve"
    elif last >= first + 5:
        direction = "worsen"
    else:
        direction = "stay mostly steady"
    return (
        f"{city_label} is expected to {direction} from about {first} AQI to {last} AQI "
        f"over the next 7 days, with roughly {spread} AQI of spread."
    )
...
def predict_7day():
    try:
        city_raw = normalize_query_text(request.args.get("city"))
        city_label = normalize_query_text(request.args.get("label") or request.args.get("display_name")) or display_label_from_query(city_raw) or "Your location"

        seed_aqi = _to_float_or_none(request.args.get("aqi"))
        if seed_aqi is None:
            seed_aqi = _to_float_or_none(request.args.get("current_aqi"))

        if seed_aqi is None and city_raw:
            live_row = fetch_live_city_row(city_raw, allow_cached_payload=True)
            if live_row:
                update_live_histories([live_row])
                seed_aqi = _to_float_or_none(live_row.get("aqi"))

        history_entries = _collect_forecast_history(city_raw, lookback_hours=168)
        if seed_aqi is None and history_entries:
            seed_aqi = _to_float_or_none(history_entries[-1].get("aqi"))

        if seed_aqi is None:
            seed_aqi = 50.0

        weather = {
            "temperature": _to_float_or_none(request.args.get("temperature") or request.args.get("temp")),
            "humidity": _to_float_or_none(request.args.get("humidity")),
            "wind_speed": _to_float_or_none(request.args.get("wind_speed") or request.args.get("wind")),
        }

        forecast_rows = _generate_7day_forecast_series(seed_aqi, history_entries, weather=weather, days=7)
        summary = _summarize_7day_forecast(city_label, forecast_rows)

        return jsonify({
            "status": "ok",
            "city": city_label,
            "base_aqi": int(round(float(seed_aqi))),
            "forecast": forecast_rows,
            "summary": summary,
            "method": "LSTM-style trend smoothing",
            "history_points": len(history_entries),
            "source": "live_history" if history_entries else "seed_aqi",
        })
    except Exception as e:
